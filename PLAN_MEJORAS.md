...
- [x] ✍️ **Toggle de ligaduras** + ajuste de peso/espaciado por fuente. Switch "Font ligatures" en Settings (default off; `body` usa `font-variant-ligatures: var(--typer-ligatures)`). `FONT_METRICS` con `letterSpacing`/`weight` por fuente (weight solo en caras variables: Fira 450, Roboto 450; el resto 400), aplicados vía `--typer-letter-spacing`/`--typer-font-weight` en el `WordSpan` del área de tipeo (reemplaza `tracking-wide`).
...
- [x] ✍️ **Atajos de teclado configurables** (defaults OFF): todos los shortcuts globales (`d` tema, `m` modo, `p/n/c/l/i` opciones, `1-4` duración) pasan de hardcodeados a `settings.shortcuts` (mapa `actionId → binding` canónico `"ctrl+shift+k"`, default `{}` = sin atajos activos hasta configurar). `ShortcutManager` global (src/components/ShortcutManager.tsx) escucha keydown con guard `isEditableTarget` ("el tipeo siempre gana"), `preventDefault` + despacha `CustomEvent("typerreflex-shortcut", {detail: actionId})`; el tema lo maneja el manager y las acciones de tipeo `TypingTest` (solo idle). Recorder en Settings (captura próxima tecla con capture:true + preventDefault/stopPropagation, Esc cancela, Backspace/Delete limpia). Helpers puros en `src/lib/shortcuts.ts` (`parseBinding`/`serializeBinding`/`matchesBinding`/`eventToBinding`/`bindingLabel`/`isEditableTarget`/`sanitizeShortcuts`). Unit en `shortcuts.test.ts` + E2E `e2e/shortcuts.spec.ts`.
- [x] ✍️ **Theme-color PWA dinámico**: el color de la pestaña/browser-chrome (`<meta name="theme-color">`) refleja el fondo real del tema (dark/light + themes de fondo lavender/sage/ocean/sand) en vez del gris fijo. `src/lib/theme-color.ts`: `oklchToHex` (conversión OKLab→sRGB, tolera el formato serializado del browser `oklch(13% .018 16)` con L en %), `resolveThemeColor` (lee `--background` computado del root → single source of truth) y `syncThemeColor` (escribe el meta). Se dispara desde `ThemeProvider.applyTheme` (toggle + cambio de sistema) y desde el effect de `SettingsProvider` (tras `applyCssVars`). Anti-flash: el script inline de `index.html` setea el meta temprano (dark `#0d0505` / light `#eeeeee`). Manifest PWA (`vite.config.ts`) → `#0d0505`. Unit en `theme-color.test.ts` + E2E en `persistence.spec.ts` (toggle light/dark + lavender dark→light).
